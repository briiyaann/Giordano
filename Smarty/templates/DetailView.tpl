...
<table width="100%" cellpadding="2" cellspacing="0" border="0">
	<tr>
		<td>
			{if $MODULE eq 'Billing' || $MODULE eq 'Payments' || $MODULE eq 'AdvancePayment' || $MODULE eq 'MemoAdvice' || $MODULE eq 'House' || $MODULE eq 'Cars'}
			<br>
			<TABLE border=0 cellspacing=0 cellpadding=0 width=100% class=small>
				<tr><td style="height:2px"></td></tr>
					<tr>
						<td style="padding-left:15px;padding-right:50px" class="moduleName" nowrap><a class="hdrLink" href="index.php?action=DetailView&module=HomeOwner&record={$HOMEOWNERID}&parenttab={$CATEGORY}">[ Back to Homeowner - {$HOMEOWNERENTITY}]</a></td>
					</tr>
				<tr><td style="height:2px"></td></tr>
			</TABLE>
			<br>
						...
						<br>
							<table align="center" border="0" cellpadding="0" cellspacing="0" width="95%">
								<tr><td>
									...
									<span class="dvHeaderText">[ {$USE_ID_VALUE} ] {$NAME} -  {$SINGLE_MOD|@getTranslatedString:$MODULE} {$APP.LBL_INFORMATION}</span>&nbsp;&nbsp;&nbsp;<br><span class="small">{$UPDATEINFO}</span>&nbsp;<span id="vtbusy_info" style="display:none;" valign="bottom"><img src="{'vtbusy.gif'|@vtiger_imageurl:$THEME}" border="0"></span><span id="vtbusy_info" style="visibility:hidden;" valign="bottom"><img src="{'vtbusy.gif'|@vtiger_imageurl:$THEME}" border="0"></span>
								...
								<td>
									<table border=0 cellspacing=0 cellpadding=3 width=100% class="small">
										<tr>
											...
											{if $REPORT eq 'true'}
											<td class="dvtUnSelectedCell" align=center nowrap><a href="index.php?action=ReportsPerHomeOwner&module={$MODULE}&record={$ID}&parenttab={$CATEGORY}">{$APP.LBL_REPORTSHOMEOWNER}</a></td>
											<td class="dvtTabCache" style="width:10px">&nbsp;</td>
											{/if}
											...
											{if $SinglePane_View eq 'false' && $IS_REL_LIST neq false && $IS_REL_LIST|@count > 0}
												...
												<td class="dvtUnSelectedCell" onmouseout="fnHideDrop('More_Information_Modules_List');" onmouseover="fnDropDown(this,'More_Information_Modules_List');" align="center" nowrap>
													<a href="index.php?action=CallRelatedList&module={$MODULE}&record={$ID}&parenttab={$CATEGORY}{$EXT}">{$APP.LBL_MORE} {$APP.LBL_INFORMATION}</a>
													<!--
											...
											<td class="dvtTabCache" align="right" style="width:100%">
												...
												{/if}
												{if $EDIT_DUPLICATE eq 'not-permitted' && $MODULE neq 'Documents'}<!--clint  remove Duplicate button -->
															...
															<table border=0 cellspacing=0 cellpadding=0 width=100%>
																...
																<form action="index.php" method="post" name="DetailView" id="form">
																	...
																	{foreach key=header item=detail from=$BLOCKS}
																		{* ed edited hiding of Blocks *}
																		{if is_array($hideBlocksTPL) && $header|in_array:$hideBlocksTPL}
																			{assign var="hidingStatBlock" value="style='display:none'"}
																		{else}
																			{assign var="hidingStatBlock" value=""}
																		{/if}
																		{* ed edited end *}
																		
																		<tr {$hidingStatBlock}><td style="padding:5px">
																					...
																					{if $BLOCKINITIALSTATUS[$header] eq 1}
																						...
																						{/if}
																								...
																								{foreach item=detail from=$detail}
																									<tr style="height:25px">
																										{foreach key=label item=data from=$detail}
																											...
																											{* ed edited hiding fields *}
																											{if is_array($hideFieldsTPL) && $keyfldname|in_array:$hideFieldsTPL}
																											...
																											{if $label ne ''}
																												{if $keycntimage ne ''}
																													<td {$hidingStatField} class="dvtCellLabel" align=right width=25%><input type="hidden" id="hdtxt_IsAdmin" value={$keyadmin}></input>{$keycntimage}</td>
																												{elseif $keyid eq '71' || $keyid eq '72'}<!-- Currency symbol -->
																													<td {$hidingStatField} class="dvtCellLabel" align=right width=25%>{$label}<input type="hidden" id="hdtxt_IsAdmin" value={$keyadmin}></input> ({$keycursymb})</td>
																													{elseif $keyid eq '9'}
																													<td {$hidingStatField} class="dvtCellLabel" align=right width=25%><input type="hidden" id="hdtxt_IsAdmin" value={$keyadmin}></input>{$label} {$APP.COVERED_PERCENTAGE}</td>
																													{elseif $keyid eq '14'}
																													<td {$hidingStatField} class="dvtCellLabel" align=right width=25%>{$label}<input type="hidden" id="hdtxt_IsAdmin" value={$keyadmin}></input> {"LBL_TIMEFIELD"|@getTranslatedString} </td>
																													{else}
																													<td {$hidingStatField} class="dvtCellLabel" align=right width=25%><input type="hidden" id="hdtxt_IsAdmin" value={$keyadmin}></input>{$label}</td>
																													{/if}
																							...
																							{if $MODULE eq 'HomeOwner'}
																								{if $header eq 'Primary Address Information' && $HOUSECOUNT != 0}
																									{include file="HouseList.tpl"}
																								{/if}
																								{if $header eq 'Primary Car' && $CARCOUNT != 0}
																...
																<tr>
																	<td style="padding:5px">
																		{if $MODULE eq 'Billing' || $MODULE eq 'MemoAdvice'}
																			{include file='PaymentTrack.tpl'}
																		{/if}
																		
																		{if $MODULE eq 'House'}
																			{include file='GoogleMapDetail.tpl'}
																		{/if}
																	</td>
																</tr>
											...
											<td width=22% valign=top style="border-left:1px dashed #cccccc;padding:13px">
												...
												<table width="100%" border="0" cellpadding="5" cellspacing="0">
													...
													{if $MODULE eq 'HomeOwner'}
														<tr>
															<td align="left" style="padding-left:10px;">
																<a class="webMnu" href="index.php?module=Billing&action=EditView&homeowner={$ID}"><img src="{'convert.gif'|@vtiger_imageurl:$THEME}" hspace="5" align="absmiddle"  border="0"/></a>
																<a class="webMnu" href="index.php?module=Billing&action=EditView&homeowner={$ID}">Add Billing</a>
															</td>
														</tr>
														<tr>
															<td align="left" style="padding-left:10px;">
																<a class="webMnu" href="index.php?module=MemoAdvice&action=EditView&homeowner={$ID}"><img src="{'convert.gif'|@vtiger_imageurl:$THEME}" hspace="5" align="absmiddle"  border="0"/></a>
																<a class="webMnu" href="index.php?module=MemoAdvice&action=EditView&homeowner={$ID}">Add Memo Advice</a>
															</td>
														</tr>
														<tr>
															<td align="left" style="padding-left:10px;">
																<a class="webMnu" href="index.php?module=Payments&action=EditView&homeowner={$ID}"><img src="{'convert.gif'|@vtiger_imageurl:$THEME}" hspace="5" align="absmiddle"  border="0"/></a>
																<a class="webMnu" href="index.php?module=Payments&action=EditView&homeowner={$ID}">Add Payment</a>
															</td>
														</tr>
														<tr>
															<td align="left" style="padding-left:10px;">
																<a class="webMnu" href="index.php?module=AdvancePayment&action=EditView&homeowner={$ID}"><img src="{'convert.gif'|@vtiger_imageurl:$THEME}" hspace="5" align="absmiddle"  border="0"/></a>
																<a class="webMnu" href="index.php?module=AdvancePayment&action=EditView&homeowner={$ID}">Add Advance Payment</a>
															</td>
														</tr>
														<tr>
															<td align="left" style="padding-left:10px;">
																<a class="webMnu" href="index.php?module=House&action=EditView&homeowner={$ID}"><img src="{'convert.gif'|@vtiger_imageurl:$THEME}" hspace="5" align="absmiddle"  border="0"/></a>
																<a class="webMnu" href="index.php?module=House&action=EditView&homeowner={$ID}">Add House</a>
															</td>
														</tr>
														<tr>
															<td align="left" style="padding-left:10px;">
																<a class="webMnu" href="index.php?module=Cars&action=EditView&homeowner={$ID}"><img src="{'convert.gif'|@vtiger_imageurl:$THEME}" hspace="5" align="absmiddle"  border="0"/></a>
																<a class="webMnu" href="index.php?module=Cars&action=EditView&homeowner={$ID}">Add Car</a>
															</td>
														</tr>
													<tr><td align="left" style="padding-left:10px;"><br></td></tr>
													<tr><td align="left" class="genHeaderSmall">{$APP.LBL_STATUSHOMEOWNER}</td></tr>
														<tr>
															<td align="left" style="padding-left:10px;">Pending Bills: {if $PENDINGBILLS eq '0'}0{else}<span style="color:red">{$PENDINGBILLS}</span>{/if}</td>
															<td align="left"><button id="viewBill" class="crmButton small save" >View</button></td>
														</tr>
														<tr>
															<td align="left" style="padding-left:10px;">Bills Amt: (Php) {if $PENDINGAMOUNT eq ''}0.00{else}<span style="color:red">{$PENDINGAMOUNT}</span>{/if}</td>
														</tr>
														<tr><td align="left" style="padding-left:10px;"><br></td></tr>
														<tr>
															<td align="left" style="padding-left:10px;">Pending Memo Advice: {if $PENDINGMEMO eq '0'}0{else}<span style="color:red">{$PENDINGMEMO}</span>{/if}</td>
															<td align="left"><button id="viewMemo" class="crmButton small save" >View</button></td>
														</tr>
														<tr>
															<td align="left" style="padding-left:10px;">Memo Advice Amt: (Php) {if $PENDINGAMOUNTMEMO eq ''}0.00{else}<span style="color:red">{$PENDINGAMOUNTMEMO}</span>{/if}</td>
														</tr>
														<tr><td align="left" style="padding-left:10px;"><br></td></tr>
													{/if}
													
													{if $MODULE eq 'Billing'}
														<tr>
															<td align="left" style="padding-left:10px;">
																<a class="webMnu" href="index.php?module=Payments&action=EditView&homeowner={$HOMEOWNERID}&bill_no={$ID}"><img src="{'convert.gif'|@vtiger_imageurl:$THEME}" hspace="5" align="absmiddle"  border="0"/></a>
																<a class="webMnu" href="index.php?module=Payments&action=EditView&homeowner={$HOMEOWNERID}&bill_no={$ID}">Add Payment</a>
															</td>
														</tr>
													{/if}
													
													{if $MODULE eq 'MemoAdvice'}
														<tr>
															<td align="left" style="padding-left:10px;">
																<a class="webMnu" href="index.php?module=Payments&action=EditView&homeowner={$HOMEOWNERID}&bill_no={$ID}"><img src="{'convert.gif'|@vtiger_imageurl:$THEME}" hspace="5" align="absmiddle"  border="0"/></a>
																<a class="webMnu" href="index.php?module=Payments&action=EditView&homeowner={$HOMEOWNERID}&bill_no={$ID}">Add Payment</a>
													...
													{elseif $MODULE eq 'Potentials'}
														...
														{/if}
													{elseif $TODO_PERMISSION eq 'true' || $EVENT_PERMISSION eq 'true' || $CONTACT_PERMISSION eq 'true'|| $MODULE eq 'Contacts' || ($MODULE eq 'Documents') || $MODULE eq 'HomeOwner'}
														...
														{if $MODULE eq 'Leads' || $MODULE eq 'Contacts' || $MODULE eq 'Accounts' || $MODULE eq 'HomeOwner'}
...
{if $MODULE eq 'Leads' or $MODULE eq 'Contacts' or $MODULE eq 'Accounts' or $MODULE eq 'Campaigns' or $MODULE eq 'Vendors' or $MODULE eq 'HomeOwner'}
