...
<?php else: ?>
	...
	if (typeof jQuery != \'undefined\') {
		jQuery(document).ready(function(){
			jQuery.fn.secureSaving = function(){
				var B = jQuery(this); 
				B.submit(function(e){
					e.preventDefault();
					jQuery.fn.undisableFields();
					VtigerJS_DialogBox.block();
					B.attr(\'action\', \'index.php\');
					B.unbind(\'submit\').submit();
				});
			}
			jQuery(\'#secureSaving\').secureSaving();
			jQuery.fn.undisableFields = function(){
				jQuery(\'form\').find(\'input, textarea, button, select\').removeAttr(\'disabled\');
			}
		});
	} 
	</script>
