...
if($_REQUEST["internal_mailer"] == "true") {
	...
	if($rec_type == "record_id") {
		...
		elseif ($type == "HomeOwner")
			$q = "select $fieldname from vtiger_homeowner where homeownerid= ?";
		else {
			...
			if(!empty($module_focus->customFieldTable)) {
				echo $q = "select $fieldname from " . $module_focus->customFieldTable[0] . " where " . $module_focus->customFieldTable[1]. "= ?";
			}
			// END
		}
