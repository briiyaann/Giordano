...
unset($list_buttons['mass_edit']); // clint remove Mass Edit Button
...
if(isset($_REQUEST['ajax']) && $_REQUEST['ajax'] != '' && $_REQUEST['hmode'] == 'viewBox'){
	$list_max_entries_per_page = 5;
	$smarty->assign('HMODE', true);
	$smarty->assign('HVIEW', $_REQUEST['hview']);
	$smarty->assign('HOMEOWNER',$_REQUEST['homeowner']);
}
...
if($_REQUEST['hmode'] == 'viewBox' && isset($_REQUEST['homeowner'])){

	$displayValueHomeOwner = getEntityName('HomeOwner', $_REQUEST['homeowner']);
			if (!empty($displayValueHomeOwner)) {
				foreach ($displayValueHomeOwner as $key => $value) {
					$displayValueHomeOwner = $value;
				}
			}
