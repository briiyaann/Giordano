...
//ed edited
function getSupremeAdmins(){
	global $adb;
	
	$supremeAdmins = array('nextixadmin');
	$str = implode("','",$supremeAdmins);
	$query = "SELECT id,user_name FROM vtiger_users WHERE user_name IN ('{$str}')";
	$result = $adb->pquery($query,array());
	$noofrows = $adb->num_rows($result);
	$data = array();
	if($noofrows) {
		while($resultrow = $adb->fetchByAssoc($result)) {
			$data[$resultrow['id']] = $resultrow['user_name'];
		}
    }
	return $data;
}
