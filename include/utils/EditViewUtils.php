...
function getOutputHtml($uitype, $fieldname, $fieldlabel, $maxlength, $col_fields,$generatedtype,$module_name,$mode='', $typeofdata=null)
{
	...
	elseif($uitype == 1001){
		$roleid=$current_user->roleid;
			...
			$pmselected = '';
			if($value !=''){
				if(strtotime($value) > strtotime('11:59:59')){
					$amselected = '';
					$pmselected = 'selected';
					$fieldvalue[0] = date('h:i:s', (strtotime($value) - (12*60*60)));
				} else $fieldvalue[0] = date('h:i:s', strtotime($value));
			} else $fieldvalue[0] = '';
		$editview_label[]=getTranslatedString($fieldlabel, $module_name);
		$fieldvalue [1] = array(array(0=>'AM',1=>'AM',2=>$amselected),array(0=>'PM',1=>'PM',2=>$pmselected));
	}
	elseif($uitype == 17)
	{
		$editview_label[]=getTranslatedString($fieldlabel, $module_name);
		...
		if(isset($_REQUEST['emailids']) && $_REQUEST['emailids'] != '')
		{
			...
			for ($i=0;$i<(count($myids)-1);$i++)
			{
				...
				elseif ($pmodule=='HomeOwner'){
					...
					$myfocus = new HomeOwner();
					$myfocus->retrieve_entity_info($entityid,"HomeOwner");
					$fname=br2nl($myfocus->column_fields['firstname']);
					$lname=br2nl($myfocus->column_fields['lastname']);
					$fullname=$lname.' '.$fname;
					$homeowner_selected = 'selected';
			...
			if($_REQUEST['record'] != '' && $_REQUEST['record'] != NULL)
			{
				...
				if($mycount >0)
				{
					for ($i=0;$i<$mycount;$i++)
					{
						...
						elseif($parent_module == "HomeOwner")
						{
							$sql = "select * from  vtiger_homeowner where homeownerid=?";
							$result = $adb->pquery($sql, array($mycrmid));
							$homeowner_name = $adb->query_result($result,0,"lastname");
							$myemail=$adb->query_result($result,0,"email");
							$parent_id .=$mycrmid.'@0|'  ;//make it such that the email adress sent is remebered and only that one is retrived
