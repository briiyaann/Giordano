			...
			foreach ($focus->list_fields as $name => $tableinfo) {
				...
				if ($is_admin == true || $profileGlobalPermission[1] == 0 || $profileGlobalPermission[2] == 0 || in_array($fieldname, $field) || $fieldname == '' || ($name == 'Close' && $module == 'Calendar')) {
					if ($fieldname == '') {
						...
						if (($module == 'Calendar' || $module == 'Emails' || $module == 'HelpDesk' || $module == 'Invoice' || $module == 'Leads' || $module == 'Contacts') && (($fieldname == 'parent_id') || ($name == 'Contact Name') || ($name == 'Close') || ($fieldname == 'firstname'))) {
						...
						} elseif ($module == 'SampleModule' && $fieldname == 'z_sm_shiftstart') {
							...
							$convtime = strtotime($value);
							if($convtime !=''){
								if($convtime > strtotime('11:59:59'))
									$value = date('h:i:s', ($convtime - (12*60*60))).' PM';
								else 
									$value = date('h:i:s', ($convtime)).' AM';
							} else $value= '';
					...
					if ($fieldname == '') {
						...
						elseif (isset($focus->popup_fields) && in_array($fieldname, $focus->popup_fields)) {
							...
							if (isset($forfield) && $forfield != '' && $focus->popup_type != 'detailview') {
								...
								if(method_exists($focus,"autoUpdateField")){
									$value = $focus->autoUpdateField($entity_id,$value,$forfield,$count,$value1);
								}else{
			...
			if ($mode == "search") {
				...
				elseif ($popuptype == "set_return_emails") {
					if ($module == 'Accounts') {
						...
						$email_check = 1;
						$count = counterValue();
						$value = '<a href="#" onclick=\'return set_return_emails(' . $entity_id . ',-1,"' . decode_html($slashes_name) . '","' . $emailaddress . '","' . $emailaddress2 . '","' . $email_check . '"); \'id = ' . $count . '>' . textlength_check($name) . '</a>';
					}
